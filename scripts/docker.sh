...
case "$1" in
    build)
        docker build --platform linux/amd64 -t miraya .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=miraya)" ]; then
            echo "Container 'miraya' is already running. Stopping it first."
            docker stop miraya
            docker rm miraya
        fi
    ...
    bash)
        ...
        if [ "$(docker ps -q -f name=miraya)" ]; then
            docker exec -it miraya bash
        else
            echo "Container 'miraya' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
